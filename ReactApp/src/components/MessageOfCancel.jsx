...
const MessageOfCancel = ({ cancels }) => {
    ...
    useEffect(() => {
        async function fetchCancelsDetails() {if(cancels.length>0){
  ...
  { haveMessage?  <div className="main-component">
    {cancelsDetails.map((cancel, index) => (
      ...
      <button className='btnPost' onClick={handleAgreeCancle}>I am agree</button>
  </div> :
